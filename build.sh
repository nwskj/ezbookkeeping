...
build_backend() {
    ...
    backend_build_extra_arguments="-X main.Version=$VERSION -X main.CommitHash=$COMMIT_HASH"
    if [ "$RELEASE" = "0" ]; then
...
build_package() {
    ...
    rm -rf package
    mkdir -p package/data package/storage package/log package/public

    cp ezbookkeeping package/
    # Adjust the following if your frontend build outputs elsewhere:
